...
from src.auth import validate_partner_role, JWTPayload
from src.models import (
    RedeemRequest, PromotionRequest, PromotionResponse, 
    PromotionListResponse, RedeemResponse, ReportResponse, BaseResponse
)
from src.db import firestore_client, postgres_client, with_circuit_breaker
from src.utils import logger, limiter, validate_cpf, hash_cpf
from src.config import RATE_LIMIT_REDEEM, CPF_HASH_SALT
    ...
    try:
        ...
        if not codes_result.get("items"):
            raise HTTPException(
                ...
                detail={"error": {"code": "INVALID_CODE", "msg": "Código não encontrado"}}
            )
        ...
        if not student:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail={"error": {"code": "STUDENT_NOT_FOUND", "msg": "Aluno não encontrado"}}
            )
        ...
        if student.get("cpf_hash") != cpf_hash:
            raise HTTPException(
                status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                detail={"error": {"code": "INVALID_CPF", "msg": "CPF não corresponde ao aluno"}}
            )
        ...
        raise HTTPException(
            ...
            detail={"error": {"code": "SERVER_ERROR", "msg": "Erro ao resgatar código"}}
        )
...
async def list_partner_promotions(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail={"error": {"code": "SERVER_ERROR", "msg": "Erro ao listar promoções"}}
        )
...
async def create_promotion(
    ...
    try:
        ...
        if request.valid_from >= request.valid_to:
            raise HTTPException(
                status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                detail={"error": {"code": "INVALID_DATES", "msg": "Data de início deve ser anterior à data de término"}}
            )
...
async def update_promotion(
    ...
    try:
        ...
        promotion = await with_circuit_breaker(get_firestore_promotion, get_postgres_promotion)
        
        if not promotion:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail={"error": {"code": "NOT_FOUND", "msg": "Promoção não encontrada"}}
            )
        
        if promotion.get("partner_id") != partner_id:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail={"error": {"code": "FORBIDDEN", "msg": "Acesso negado a esta promoção"}}
            )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail={"error": {"code": "SERVER_ERROR", "msg": "Erro ao atualizar promoção"}}
        )
...
async def delete_promotion(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail={"error": {"code": "SERVER_ERROR", "msg": "Erro ao desativar promoção"}}
        )
...
async def get_partner_reports(
    ...
    try:
        ...
        if not re.match(r"^\d{4}-\d{2}$", range):
            raise HTTPException(
                status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                detail={"error": {"code": "INVALID_RANGE", "msg": "Formato de período inválido, use YYYY-MM"}}
            )
        ...
        used_codes = [code for code in codes_result.get("items", []) if code.get("used_at")]
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail={"error": {"code": "SERVER_ERROR", "msg": "Erro ao gerar relatório"}}
        )
