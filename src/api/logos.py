...
async def list_partner_logos(
    ...
    """
    Lista todos os logos de parceiros disponíveis (apenas administradores).
    ...
    try:
        logger.info(
            f"Admin {current_user.sub} solicitou listagem de logos - categoria: {category}, force_refresh: {force_refresh}"
        )
        ...
        logger.info(f"Retornando {len(logos_data)} logos para admin {current_user.sub}")
        return logos_data

    except Exception as e:
        logger.error(f"Erro ao listar logos para admin {current_user.sub}: {e}")
        raise HTTPException(
...
async def get_partner_logo(
    ...
    """
    Obtém o logo de um parceiro específico (apenas administradores).
    ...
    try:
        logger.info(
            f"Admin {current_user.sub} solicitou logo do parceiro {partner_id}"
        )
        ...
        logger.info(f"Logo encontrado para parceiro {partner_id} (solicitado por admin {current_user.sub})")
        ...
        logger.error(f"Erro inesperado ao buscar logo do parceiro {partner_id} para admin {current_user.sub}: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail={
...
async def list_logo_categories(
    ...
    """
    Lista todas as categorias de logos disponíveis (apenas administradores).
    ...
    """
    try:
        logger.info(f"Admin {current_user.sub} solicitou lista de categorias")
        ...
        logger.info(f"Encontradas {len(categories)} categorias para admin {current_user.sub}: {categories}")

        return categories

    except Exception as e:
        logger.error(f"Erro ao listar categorias para admin {current_user.sub}: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail={
                "error": {"code": "INTERNAL_ERROR", "msg": "Erro interno do servidor"}
...
async def refresh_logo_urls(
    ...
    """
    Força a regeneração de todas as URLs de logos (apenas administradores).
    ...
    """
    try:
        logger.info(f"Admin {current_user.sub} solicitou refresh das URLs de logos")

        # Regenerar URLs
        refreshed_count = await logos_service.refresh_expired_urls()

        logger.info(f"Refresh concluído por admin {current_user.sub}: {refreshed_count} URLs regeneradas")
        ...
        logger.error(f"Erro ao fazer refresh das URLs para admin {current_user.sub}: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail={
                "error": {
...
async def clear_logos_cache(
    ...
    """
    Limpa o cache de logos (apenas administradores).
    ...
    try:
        ...
        logger.error(f"Erro ao limpar cache para admin {current_user.sub}: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail={
                "error": {"code": "INTERNAL_ERROR", "msg": "Erro interno do servidor"}
...
async def logos_health_check(
    ...
    """
    Verifica a saúde do serviço de logos (apenas administradores).
    ...
    """
    try:
        logger.debug(f"Health check solicitado por admin {current_user.sub}")
        ...
        logger.error(f"Erro no health check para admin {current_user.sub}: {e}")
