...
class MockPostgres:
    ...
    async def query_documents(
        ...
        """
        # Usar a mesma implementação do Firestore simulado
        return await MockFirestore.query_documents(table, filters, order_by, limit, offset)
    ...
    def can_execute(self):
        ...
        if self.state == "open":
            ...
            if time.time() - self.last_failure_time > 10:
                ...
                print("Circuit breaker em estado half-open, tentando Firestore novamente")
                return True
            return False
...
def toggle_firestore_failure_mode():
    ...
    print(f"Modo de falha do Firestore: {'ATIVADO' if FIRESTORE_FAILURE_MODE else 'DESATIVADO'}")

if __name__ == "__main__":
    print("Módulo de simulação de banco de dados para testes locais.")
    print("Use as classes MockFirestore, MockPostgres e MockCircuitBreaker para testes.")
    print("Use a função toggle_firestore_failure_mode() para simular falhas no Firestore.")
