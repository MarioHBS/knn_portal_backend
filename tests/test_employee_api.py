...
class TestEmployeePartnersEndpoints:
    ...
    async def test_list_partners_success(self, mock_employee_user, mock_partner):
        ...
        with patch("src.api.employee.validate_employee_role", return_value=mock_employee_user), \
             patch("src.api.employee.PartnersService.list_partners_common") as mock_service:
    ...
    @pytest.mark.asyncio
    async def test_list_partners_with_filters_and_ordering(self, mock_employee_user, mock_partner):
    ...
    @pytest.mark.asyncio
    async def test_list_partners_default_ordering(self, mock_employee_user, mock_partner):
    ...
    @pytest.mark.asyncio
    async def test_get_partner_details_success(self, mock_employee_user, mock_partner, mock_promotion):
...
class TestEmployeeHistoryEndpoint:
    ...
    @pytest.mark.asyncio
    async def test_get_employee_validation_history_success(self, mock_employee_user, mock_validation_code, mock_partner):
    ...
    @pytest.mark.asyncio
    async def test_get_employee_validation_history_partner_not_found(self, mock_employee_user, mock_validation_code):
        ...
        with patch("src.api.employee.validate_employee_role", return_value=mock_employee_user), \
            ...
            assert result["msg"] == "ok"
            assert result["data"]["total"] == 1
            assert result["data"]["history"][0]["partner"]["name"] == "Parceiro não encontrado"

    @pytest.mark.asyncio
    async def test_get_employee_validation_history_service_error(self, mock_employee_user):
...
class TestEmployeeValidationCodeEndpoint:
    ...
    @pytest.mark.asyncio
    async def test_create_validation_code_success(self, mock_employee_user, mock_partner, mock_employee):
        ...
        with patch("src.api.employee.validate_employee_role", return_value=mock_employee_user), \
            ...
            mock_business_rules.calculate_code_expiration.return_value = datetime.now() + timedelta(minutes=3)
    ...
    @pytest.mark.asyncio
    async def test_create_validation_code_partner_inactive(self, mock_employee_user, mock_partner):
    ...
    @pytest.mark.asyncio
    async def test_create_validation_code_employee_not_found(self, mock_employee_user, mock_partner):
    ...
    @pytest.mark.asyncio
    async def test_create_validation_code_employee_inactive(self, mock_employee_user, mock_partner, mock_employee):
        """Testa criação de código com funcionário inativo."""
        mock_employee["active_until"] = (datetime.now() - timedelta(days=1)).date().isoformat()
    ...
    @pytest.mark.asyncio
    async def test_create_validation_code_duplicate_handling(self, mock_employee_user, mock_partner, mock_employee):
        ...
        with patch("src.api.employee.validate_employee_role", return_value=mock_employee_user), \
            ...
            # Mock para regras de negócio
            mock_business_rules.validate_student_active.return_value = True
            mock_business_rules.generate_validation_code.side_effect = ["123456", "654321"]  # Dois códigos diferentes
            mock_business_rules.calculate_code_expiration.return_value = datetime.now() + timedelta(minutes=3)
