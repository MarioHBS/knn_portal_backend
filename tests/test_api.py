...
def client():
    ...
    # Rotas para testes
    @app.get("/v1/partners")
    async def list_partners(current_user = Depends(lambda: get_current_user(role="student"))):
        return {
            "data": {
                "items": [
    ...
    async def get_partner(id: str, current_user = Depends(lambda: get_current_user(role="student"))):
        if id == "not-found":
    ...
    async def create_validation_code(request: dict, current_user = Depends(lambda: get_current_user(role="student"))):
        return {
            "data": {
                "code": "123456",
    ...
    async def redeem_code(request: dict, current_user = Depends(lambda: get_current_user(role="partner"))):
        if request.get("code") == "invalid":
            raise HTTPException(status_code=404, detail={"error": {"code": "NOT_FOUND", "msg": "Código não encontrado"}})
        if request.get("code") == "expired":
            raise HTTPException(status_code=410, detail={"error": {"code": "EXPIRED", "msg": "Código expirado ou já usado"}})
        return {
            "data": {
                "student": {
                    "name": "Aluno Teste",
    ...
    async def redeem_with_rate_limit(request: dict, current_user = Depends(lambda: get_current_user(role="partner"))):
        ...
        if _rate_limit_counter["count"] > 5:
            raise HTTPException(status_code=429, detail={"error": {"code": "RATE_LIMIT", "msg": "Rate limit excedido"}})
...
def test_invalid_token(client):
    ...
    response = client.get("/v1/authenticated", headers={"Authorization": "Bearer invalid"})
    assert response.status_code == 401
    assert response.json()["error"]["code"] == "UNAUTHORIZED"
...
def test_wrong_role(client):
    ...
    response = client.get("/v1/authenticated", headers={"Authorization": "Bearer wrong-role"})
...
def test_list_partners(client):
    ...
    response = client.get("/v1/partners", headers={"Authorization": "Bearer valid-student"})
    assert response.status_code == 200
...
def test_get_partner_details(client):
    ...
    partner_id = str(uuid.uuid4())
    response = client.get(f"/v1/partners/{partner_id}", headers={"Authorization": "Bearer valid-student"})
    assert response.status_code == 200
...
def test_partner_not_found(client):
    ...
    response = client.get("/v1/partners/not-found", headers={"Authorization": "Bearer valid-student"})
...
def test_admin_metrics(client):
    ...
    response = client.get("/v1/admin/metrics", headers={"Authorization": "Bearer valid-admin"})
    assert response.status_code == 200
...
def test_cpf_validation():
    ...
    from src.utils import validate_cpf, hash_cpf
